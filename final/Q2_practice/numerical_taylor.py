...
def show_heat_map(data2D, time_lbl):
    N_data = data2D.shape[0]
    fg = plt.figure()
    p = int(321)
    for i in np.linspace(1, N_data, 6):
        i = int(i)-1
        print(i)
        plt.subplot(p)
        # ax = sns.heatmap(data2D[i].real, cmap="RdBu_r") 
        ax = sns.heatmap(data2D[i].real, robust=True, fmt="f", cmap='RdBu_r', vmin=-1.0, vmax=3.0) 
        plt.title('t={}'.format(time_lbl[i]))
        p += 1
    return fg
